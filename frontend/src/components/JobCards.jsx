...
let JobCards = ({job,updateStatus, deleteJob, onEdit})=>{
  return (
    <div className={`${style.cards}`}>
      <div className={`${style.Jobinfo}`}>
       ...
       <label className={style.statusLabel}>Status</label>
        <select className={style.statusSelect}
          value={job.status}
          onChange={(e) => updateStatus(job._id, 
          e.target.value)}>
      ...
      <div className={`${style.editBtns}`}>
        ...
        <div className={`${style.delete} btn btn-outline-danger`} onClick ={()=>deleteJob(job._id)}  ><i className="bi bi-trash"></i></div>
      </div>
    </div>
  )
}
